/* ════════════════════════════════════════════════════════════════════
   ...
   Credential store: IndexedDB "R1DOToolsDB" (auto-migrates records
   from the legacy "R1DONotesDB" on first open).

   Legacy helpers reproduce the old per-app derivations exactly, so
   each tool can decrypt pre-existing data once (via the PRF `second`
   evaluation) and re-encrypt it under the unified scheme.
   ════════════════════════════════════════════════════════════════════ */
...
export const LEGACY_DB_NAME = "R1DONotesDB";
...
// Legacy per-app PRF salts — only used as the PRF `second` input to
// migrate old data. Never used for new keys.
export const LEGACY_PRF_SALT_NOTES =
    "R1DO, derive my super keypair from this deterministic signature to encrypt notes";
export const LEGACY_PRF_SALT_TASKS = "r1dotasks-prf-v1";
...
/* Record format (kept compatible with the old R1DONotesDB):
...
async function _migrateFromLegacyDB(newDb) {
    const current = await _getAll(newDb);
    if (current.length > 0) return; // already populated — nothing to do

    // Only open the legacy DB if it actually exists (open() would create it).
    ...
    try {
        const oldDb = await _openDB(LEGACY_DB_NAME, undefined, null);
        const records = await _getAll(oldDb);
        oldDb.close();
        ...
        console.log(
            `[r1do-auth] Migrated ${records.length} credential(s) from ${LEGACY_DB_NAME} → ${DB_NAME}`,
        );
    } catch (e) {
        console.warn("[r1do-auth] Legacy DB migration skipped:", e);
    }
}
...
export async function initCredDB() {
    ...
    await _migrateFromLegacyDB(_db);
    return _db;
}
...
/**
 * Authenticate and evaluate the suite PRF (plus an optional legacy salt
 * as `second`, for one-time data migrations).
 *
 ...
 *
 * Returns { prfOutput, legacyPrf, credentialId, username }.
 */
export async function authenticate({
    ...
    if (legacySalt) evalInputs.second = utf8ToBytes(legacySalt);
    ...
    const legacyPrf = results.second ? new Uint8Array(results.second) : null;
...
/** Decrypt blobs in either v2 {iv, ct} or legacy {iv, ciphertext} shape. */
export async function aesDecrypt(key, blob) {
    const iv = new Uint8Array(blob.iv);
    const ct = new Uint8Array(blob.ct ?? blob.ciphertext);
...
/* Bit-exact copies of the old derivations. Decrypt-only, for migration
   and for reading envelopes produced by pre-v2 versions. */

/** Old notes/chat identity: rejection-sample the raw legacy PRF bytes. */
export function legacyIdentityFromPrf(legacyPrf) {
    const privateKey = _toP256ScalarLegacy(legacyPrf);
    const publicKey = p256.getPublicKey(privateKey, false);
    return { privateKey, publicKey, publicKeyHex: bytesToHex(publicKey) };
}

function _toP256ScalarLegacy(seed) {
    // The old loop had no zero-guard — reproduce it exactly.
    let hash = seed;
    let attempt = 0;
    while (BigInt("0x" + bytesToHex(hash)) >= CURVE_ORDER_P256) {
        hash = sha256(concatBytes(hash, new Uint8Array([attempt++])));
    }
    return hash;
}

/** Old home-made KDF: prk = sha256(salt‖secret); okm = sha256(prk‖info‖0x01). */
function _legacyKdf(secret, salt, info) {
    const prk = sha256(concatBytes(salt, secret));
    return sha256(concatBytes(prk, info, new Uint8Array([0x01])));
}

/** Old notes at-rest AES key (derived from the legacy identity private key). */
export async function legacyNotesAesKey(legacyIdentityPrivateKey) {
    const material = _legacyKdf(
        legacyIdentityPrivateKey,
        new Uint8Array(32),
        utf8ToBytes("aes-encryption-key-v1"),
    );
...
/** Old tasks DEK: HKDF(salt "r1dotasks-v1", info "data-encryption-v1"). */
export async function legacyTasksDek(legacyPrf) {
    const key = await _importIkm(legacyPrf);
    return crypto.subtle.deriveKey(
        {
            name: "HKDF",
            hash: "SHA-256",
            salt: utf8ToBytes("r1dotasks-v1"),
            info: utf8ToBytes("data-encryption-v1"),
        },
        key,
        { name: "AES-GCM", length: 256 },
        false,
...
/**
 * Decrypt a legacy (unsigned) ECIES envelope from notes or chat.
 * Notes shape: { version:"1.0", ephemeralPublic:[], iv:[], ciphertext:[] }
 * Chat shape:  { v:"1", e:[], i:[], c:[] }
 * Old shared secret: compressed-point default → x-only 32 bytes.
 */
export async function legacyEciesDecrypt({ envelope, myPrivateKey }) {
    const isChat = envelope.v === "1";
    const ephPub = new Uint8Array(
        isChat ? envelope.e : envelope.ephemeralPublic,
    );
    const iv = new Uint8Array(isChat ? envelope.i : envelope.iv);
    const ct = new Uint8Array(isChat ? envelope.c : envelope.ciphertext);
    const infoLabel = isChat ? "r1do-chat-ecies-v1" : "ecies-encryption-v1";

    const sharedSecret = p256
        .getSharedSecret(myPrivateKey, ephPub)
        .slice(1); // x-only 32 bytes (compressed default), as the old code did
    const material = _legacyKdf(
        sharedSecret,
        new Uint8Array(32),
        utf8ToBytes(infoLabel),
    );
...
/** Dispatch: v2 signed envelope vs legacy unsigned (notes or chat shape). */
export async function decryptAnyEnvelope({ envelope, myPrivateKey, legacyPrivateKey = null }) {
    if (envelope && envelope.v === 2) {
        return eciesDecrypt({ envelope, myPrivateKey });
    }
    // Legacy envelopes were encrypted to the OLD identity.
    const key = legacyPrivateKey ?? myPrivateKey;
    const text = await legacyEciesDecrypt({ envelope, myPrivateKey: key });
    return { text, senderPublicKeyHex: null, verified: null }; // unsigned era
}

/* ───────────────── contacts (encrypted at rest, shared) ──────────── */

const CONTACTS_KEY_V2 = "contacts_v2";
const CONTACTS_KEY_LEGACY = "contacts";

/**
 * Load the contact list. Reads the encrypted v2 record; if only the old
 * plaintext record exists it is migrated (encrypted + plaintext removed).
 */
export async function loadContacts(contactsDek) {
    ...
    const legacy = localStorage.getItem(CONTACTS_KEY_LEGACY);
    if (legacy) {
        try {
            const list = JSON.parse(legacy);
            await storeContacts(contactsDek, list);
            console.log(
                `[r1do-auth] Migrated ${list.length} contact(s) to encrypted storage`,
            );
            return list;
        } catch {
            return [];
...
export async function storeContacts(contactsDek, list) {
    ...
    localStorage.removeItem(CONTACTS_KEY_LEGACY);
}
